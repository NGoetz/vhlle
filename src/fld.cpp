...
void Fluid::outputEta(TransportCoeff* trCoeff) {
  ...
  output::feta <<"average eta/s, weighted by epsilon " << trCoeff->sum_eta_s/trCoeff->sum_epsilon << "  average eta/s, weighted by epsilon and number of cells "<< trCoeff->sum_eta_s_weight*trCoeff->num/(trCoeff->sum_epsilon*trCoeff->sum_weight) <<endl;
}
